...
class LandslideProbability(Component):
    ...
    The driving force for failure is provided by the user in the form of either
    groundwater recharge or depth to groundwater; four options for providing recharge are supported.
    ...
    Option 4 - Data_driven_spatial recharge
        ...
        groundwater__recharge_HSD_inputs=[HSDr_dict, HSDr_id_dict,
        fractr_dict])
    ...
    Option 6 - Lognormal depth
        ...
        groundwater__depth_standard_deviation=1)
    Option 7 - Lognormal_spatial depth
        LandslideProbability(grid, number_of_iterations=250,
        groundwater__depth_distribution='lognormal_spatial', 
        groundwater__depth_mean=np.random.randint(0, 3, grid_size),
    ...
    Option 8 - Data_driven_spatial depth to water
        ...
        groundwater__depth_HSD_inputs=[HSDd_dict, HSDd_id_dict,
        fractd_dict])
    ...
    groundwater__recharge_HSD_inputs: list, optional
        list of 3 dictionaries in order (default=[]) - HSDr_dict {Hydrologic
        Source Domain (HSD) keys: recharge numpy array values},HSDr_id_dict {node IDs keys:
        list of HSD_Id values}, fractr_dict {node IDS keys: list of
    ...
    groundwater__depth_distribution: str, optional
        single word indicating depth distribution, either 'uniform',
        'lognormal', 'lognormal_spatial,' or 'data_driven_spatial'.
         (default='uniform')
    ...
    groundwater__depth_max_value: float, optional (mm/d)
        maximum groundwater depth for 'uniform' (default=3.)
    groundwater__depth_mean: float, optional (mm/d) 
        mean grounwater depth for 'lognormal'
        and 'lognormal_spatial' (default=None)
    ...
    groundwater__depth_HSD_inputs: list, optional
        list of 3 dictionaries in order (default=[]) - HSDd_dict {Hydrologic
        Source Domain (HSD) keys: depth numpy array values}, HSDd_id_dict {node IDs keys:
        list of HSD_Id values}, fractd_dict {node IDS keys: list of
        HSD fractions values} (none) - for more information refer to
        Ref 1 & Ref 2 mentioned above, as this set of inputs require
        rigorous pre-processing of data.
   ...
   >>> grid.at_node['soil__mode_total_cohesion'] = np.sort(
    ...
    _var_doc = {
        ...
        'soil__mean_relative_wetness':
            ('Indicator of soil wetness;' +
             ' relative depth perched groundwater' +
    ...
    def __init__(self, grid, number_of_iterations=250,
        ...
        groundwater__recharge_standard_deviation: float, optional (mm/d)
            ...
            and 'lognormal_spatial' (default=None)
        groundwater__recharge_HSD_inputs: list, optional
            list of 3 dictionaries in order (default=[]) - HSDr_dict
            {Hydrologic Source Domain (HSD) keys: recharge numpy array values},
            HSDr_id_dict {node IDs keys: list of HSD_Id values}, HSDr_fractions
        ...
        groundwater__depth_distribution: str, optional
            single word indicating depth to groundwater distribution, either 'uniform',
            'lognormal', 'lognormal_spatial,' or 'data_driven_spatial'.
             (default='uniform')
        groundwater__depth_min_value: float, optional (mm/d)
            minium groundwater depth to groundwater for 'uniform' (default=0.)
        groundwater__depth_max_value: float, optional (mm/d)
            maximum groundwater depth for 'uniform' (default=3.)
        groundwater__depth_mean: float, optional (mm/d) 
            mean grounwater depth to groundwater for 'lognormal'
            and 'lognormal_spatial' (default=None)
        groundwater__depth_standard_deviation: float, optional (mm/d)
            standard deviation of grounwater depth to groundwater for 'lognormal'
            and 'lognormal_spatial' (default=None)
        groundwater__depth_HSD_inputs: list, optional
            list of 3 dictionaries in order (default=[]) - HSDd_dict
            {Hydrologic Source Domain (HSD) keys: recharge numpy array values},
            HSDd_id_dict{node IDs keys: list of HSD_Id values}, HSDd_fractions
            {node IDS keys: list of HSD fractions values} (none) - for more
            information refer to Ref 1 & Ref 2 mentioned above, as this set of
            inputs require rigorous pre-processing of data.
        ...
        #dummy values initialized to the correct size
        ...
        if self.groundwater__recharge_distribution == 'lognormal':
        ...
        if self.groundwater__recharge_distribution == 'data_driven_spatial':
            self._HSDr_dict = groundwater__recharge_HSD_inputs[0]
            self._HSDr_id_dict = groundwater__recharge_HSD_inputs[1]
            self._fractr_dict = groundwater__recharge_HSD_inputs[2]
            self._interpolate_HSD_dict()

        # Depth to groundwater Uniform distribution
        ...
        # Depth to groundwater Lognormal Distribution - Uniform in space
        ...
        # Depth to groundwater  Lognormal Distribution - Variable in space                                  
        ...
        # Depth to groundwater Custom HSD inputs - Hydrologic Source Domain -> Model Domain
        if self.groundwater__depth_distribution == 'data_driven_spatial':
            self._HSDd_dict = groundwater__depth_HSD_inputs[0]
            self._HSDd_id_dict = groundwater__depth_HSD_inputs[1]
            self._fractd_dict = groundwater__depth_HSD_inputs[2]
            self._interpolate_HSD_dict()
    ...
    def calculate_factor_of_safety(self, i):
        ...
        # Depth to groundwater distribution based on distribution type
        ...
        if self.groundwater__recharge_distribution == 'uniform':
            ...
            self._rel_wetness = (self._De)/(self._hs_mode)
    ...
    def _calculate_HSD_depth(self, i):
        """This method calculates the resultant depth to groundwater at node i of the
        model domain, using recharge of contributing HSD ids and the areal
        fractions of upstream contributing HSD ids. Output is a numpy array
        of recharge at node i.        
        """
        store_De = np.zeros(self.n)
        HSD_id_list = self._HSD_id_dict[i]
        fract_list = self._fract_dict[i]
        for j in range(0, len(HSD_id_list)):
            De_temp = self._interpolated_HSD_dict[HSD_id_list[j]]
            fract_temp = fract_list[j]
            De_adj = (De_temp*fract_temp)
            store_De = np.vstack((store_De, np.array(De_adj)))
        self._De = np.sum(store_De, 0)        
