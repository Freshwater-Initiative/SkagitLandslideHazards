   ...
   >>> grid.at_node['soil__mode_total_cohesion'] = np.sort(
    ...
    def calculate_factor_of_safety(self, i):
        ...
        if self.groundwater__depth_distribution is not None:
            ...
            hw_dist[np.where(hw_dist<0)] = self._hs_mode  #replace saturated cells to be the max soil thickness
    ...
    def _calculate_HSD_groundwater_depth(self, i):
        ...
        dds_dist = pd.DataFrame.from_dict(groundwater__depth_HSD_inputs, orient='index')
        print('In calculate HSD loop - dds_dist.values[0]')
        print(dds_dist.values[0])
        self._De = dds_dist.values[0]
